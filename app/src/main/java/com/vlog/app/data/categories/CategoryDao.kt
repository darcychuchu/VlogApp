...
interface CategoryDao {
     ...
     */
    @Query("SELECT * FROM categories WHERE parentId IS NULL ORDER BY isTyped ASC, orderSort DESC")
    fun getMainCategories(): Flow<List<CategoryEntity>>

    /**
     * 获取指定分类的子分类
     */
    @Query("SELECT * FROM categories WHERE parentId = :parentId ORDER BY isTyped ASC, orderSort DESC")
